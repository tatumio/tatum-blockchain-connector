    ...
    public async transferErc20(body: ChainTransferEthErc20 | ChainTransferBscBep20 | ChainTransferCeloErc20Token):
        ...
        switch (chain) {
            ...
            case Currency.CELO:
                ...
                break;
            case Currency.XDC:
                txData = await prepareXdcOrErc20SignedTransaction(_body as TransferEthErc20, (await this.getFirstNodeUrl(chain, testnet)));
                break;
            default:
                throw new Erc20Error(`Unsupported chain ${chain}.`, 'unsuported.chain');
        }
    ...
    public async deployErc20(body: ChainDeployErc20 | ChainDeployCeloErc20): Promise<TransactionHash | { signatureId: string }> {
        ...
        switch (chain) {
            ...
            case Currency.CELO:
                ...
                break;
            case Currency.XDC:
                txData = await prepareXdcDeployErc20SignedTransaction(_body as DeployErc20, (await this.getFirstNodeUrl(chain, testnet)));
                break;
            default:
